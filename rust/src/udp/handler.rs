...
use log::{debug, error};
use tokio::{fs::File, io::AsyncWriteExt, net::UdpSocket, sync::Mutex};
...
pub(crate) struct UdpPacketInfo<'a> {
    ...
    pub vpn_service_java_obj: JObject<'a>,
    pub device_output_stream: Arc<Mutex<File>>,
}

pub(crate) async fn handle_udp_packet(udp_packet_info: UdpPacketInfo<'_>) -> Result<()> {
    let UdpPacketInfo {
        source_address,
        source_port,
        destination_address,
    ...
    tokio::spawn(async move {
        ...
        let receive_data_size = match local_udp_socket.recv(&mut receive_data).await {
            Ok(0) => {
