    ...
    T: AsyncWrite + Unpin + Send + 'static,
{
    ...
    async fn close_connection(&self, data_model: &TcpConnectionDataModel, send_reset: bool) {
        if send_reset {
            if let Err(e) = send_rst_to_device(self.key, data_model, self.device_write.clone()).await {
                error!(">>>> Tcp connection [{}] fail to send reset to device because of error: {e:?}", self.key)
            };
        }
    ...
    pub async fn process<'a, 'j>(&mut self, _ipv4_header: Ipv4HeaderSlice<'a>, tcp_header: TcpHeaderSlice<'a>, payload: &'a [u8]) -> Result<()> {
        ...
        match data_model.status {
            TcpConnectionStatus::Listen => {
                ...
                if let Err(e) = send_syn_ack_to_device(self.key, &data_model, self.device_write.clone()).await {
            ...
            TcpConnectionStatus::SynReceived => {
                ...
                let destination_tcp_socket = match TcpSocket::new_v4() {
                    Ok(destination_tcp_socket) => {
                        ...
                        if let Err(e) = protect_socket(format!("{}", self.key), destination_tcp_socket_raw_fd) {
                ...
                let device_output_stream_clone = self.device_write.clone();
            ...
            TcpConnectionStatus::Established => {
                ...
                if tcp_header.fin() {
                    ...
                    send_ack_to_device(self.key, &data_model, self.device_write.clone(), None).await?;
                    ...
                    send_fin_ack_to_device(self.key, &data_model, self.device_write.clone()).await?;
                    return Ok(());
                }
            ...
            TcpConnectionStatus::TimeWait => {
                ...
                if let Err(e) = send_ack_to_device(self.key, &data_model, self.device_write.clone(), None).await {
                    error!(
            ...
            TcpConnectionStatus::Closed => {
                ...
                send_rst_to_device(self.key, &data_model, self.device_write.clone()).await?;
                Ok(())
            },
        }
    }
...
async fn send_syn_ack_to_device<T>(key: TcpConnectionKey, data_model: &TcpConnectionDataModel, device_vpn_write: Arc<Mutex<T>>) -> Result<()>
where
    T: AsyncWrite + Unpin + Send + 'static,
{
    ...
    if let Err(e) = device_vpn_write.write(&sync_ack_packet_bytes).await {
...
async fn send_fin_ack_to_device<T>(key: TcpConnectionKey, data_model: &TcpConnectionDataModel, device_vpn_write: Arc<Mutex<T>>) -> Result<()>
where
    T: AsyncWrite + Unpin + Send + 'static,
{
    ...
    let mut device_vpn_write = device_vpn_write.lock().await;
    if let Err(e) = device_vpn_write.write(&rst_ack_packet_bytes).await {
...
async fn send_rst_to_device<T>(key: TcpConnectionKey, data_model: &TcpConnectionDataModel, device_vpn_write: Arc<Mutex<T>>) -> Result<()>
where
    T: AsyncWrite + Send + Unpin + 'static,
{
    let rst_ack_tcp_packet = PacketBuilder::ipv4(key.destination_address.octets(), key.source_address.octets(), IP_PACKET_TTL)
...
async fn send_ack_to_device<T>(
    key: TcpConnectionKey, data_model: &TcpConnectionDataModel, device_vpn_write: Arc<Mutex<T>>, payload: Option<&[u8]>,
) -> Result<()>
where
    T: AsyncWrite + Send + Unpin + 'static,
{
    ...
    let mut device_vpn_write = device_vpn_write.lock().await;
    if let Err(e) = device_vpn_write.write(&ack_packet_bytes).await {
...
async fn start_read_destination<T>(
    data_model: Arc<RwLock<TcpConnectionDataModel>>, key: TcpConnectionKey, device_vpn_write: Arc<Mutex<T>>, mut destination_read: OwnedReadHalf,
) -> Result<()>
where
    T: AsyncWrite + Send + Unpin + 'static,
{
